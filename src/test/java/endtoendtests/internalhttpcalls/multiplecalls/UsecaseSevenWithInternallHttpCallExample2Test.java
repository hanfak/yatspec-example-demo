...
public class UsecaseSevenWithInternallHttpCallExample2Test extends TestState implements WithCustomResultListeners {
  ...
  private GivensBuilder theStarWarsServiceReturnsCharacterInfoContaining(String birthYear, String speciesLink) {
    ...
    return interestingGivens -> {
      ...
      wiremock.register(stubGet("/people/" + PERSON_ID, primedResponse, "application/json"));
      return interestingGivens;
    };
  }
  ...
  private GivensBuilder theStarWarsServiceReturnsSpeciesInfoContaining(String hobbit) {
    return interestingGivens -> {
      String primedResponse = format(STAR_WARS_SPECIES_INFO_RESPONSE, hobbit, "http://localhost:8888/species/5");
      // Register our primings with wiremock rather than the server
      wiremock.register(stubGet("/species/5", primedResponse, "application/json"));
      return interestingGivens;
    };
  }

  private MappingBuilder stubGet(String path, String body, String contentType) {
    // As Wiremock and the server are on the same port, we can access the same primings
    // So we can register as many primings as we want
